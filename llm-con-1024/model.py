...
class HARTrans(torch.nn.Module):
    def __init__(self,args):
        ...
        # 域特定分类头设置
        if hasattr(args, 'use_domain_heads') and args.use_domain_heads:
            self.use_domain_heads = True
            self.n_domains = 5  # 5个人
            # 为每个域创建独立的分类头
            self.domain_heads = nn.ModuleList([
                nn.Linear(342, 27).to(self.device) for _ in range(self.n_domains)
            ])
        else:
            self.use_domain_heads = False
            self.feats = nn.ModuleList([self._create_feat() for _ in range(6)])
            self.feats_status = [0]
        if hasattr(args, 'use_domain_prediction') and args.use_domain_prediction:
            self.use_domain_prediction = True
            self.domain_predictor = nn.Sequential(
                nn.Linear(342, 128),
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.Linear(128, 5)  # 预测5个域 (P1到P5)
            ).to(self.device)
        else:
            self.use_domain_prediction = False
        if hasattr(args, 'use_prompt_distillation') and args.use_prompt_distillation:
            from learnable_prompt_distillation import LearnablePromptDistillation
            self.prompt_distiller = LearnablePromptDistillation(args)
            self.use_prompt_distillation = True
        else:
            self.use_prompt_distillation = False
    ...
    def forward(self, data, task=None, domains=None,labels=None):
        """
        修改 forward 函数以返回域预测结果
        """
        # 1. 提取核心的342维特征
        internal_feature_342d, task_prefix = self.extract_features(data, task)

        # 2. 进行域预测（无论域是否已知，都进行预测，损失函数会决定是否使用）
        domain_logits = self.domain_predictor(internal_feature_342d) if self.use_domain_prediction else None
        distillation_loss = None
        if self.use_prompt_distillation and self.training and labels is not None:
            distillation_loss = self.prompt_distiller.distillation_loss(
                internal_feature_342d, labels, task, domains
            )
        # 3. 使用给定的域标签（可能包含未知标签-1）进行分类
        if self.use_domain_heads and domains is not None:
            # 如果域未知（-1），暂时使用默认头（如P1），后续会在LGCLWrapper中被预测值替换
            temp_domains = domains.clone()
            temp_domains[temp_domains == -1] = 1 # 用 P1 作为临时占位符
            logits = self.forward_domain_heads(internal_feature_342d, temp_domains)
        else:
        ...
        # 5. 返回所有结果
        return logits, projected_feature_512d, task_prefix, domain_logits, distillation_loss
    
    def forward_domain_heads(self, features, domains):
        """使用域特定分类头进行前向传播"""
        batch_size = features.size(0)
        logits_list = []
        
        for i in range(batch_size):
            domain_id = domains[i].item() - 1  # 域ID从1开始，转换为0索引
            if 0 <= domain_id < self.n_domains:
                # 使用对应域的分类头
                logits = self.domain_heads[domain_id](features[i])
                logits_list.append(logits)
            else:
                # 域ID无效，使用默认（第一个）分类头
                logits = self.domain_heads[0](features[i])
                logits_list.append(logits)
    ...
    def return_hidden(self, data, task=None):
        """为了兼容性保留"""
        internal_feature_342d, _ = self.extract_features(data, task=task)
        return internal_feature_342d
        
    def forward_classifier(self, x):
        """兼容旧代码"""
        if self.use_domain_heads:
            # 在没有域信息时，使用第一个域的分类头作为默认
            return self.domain_heads[0](x)
        else:
    ...
    def forward_pre(self, x, origin=None):
        """原有的分类逻辑，用于非域特定模式"""
        if self.use_domain_heads:
            # 如果启用了域分类头但没有提供域信息，使用默认行为
            return self.domain_heads[0](x)
        
        # 原有逻辑保持不变
        if not hasattr(self, 'feats_status') or len(self.feats_status) == 1:
            return self.feats[0](x) if hasattr(self, 'feats') else self.domain_heads[0](x)
        else:
            batch_size = x.size(0)
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        ...
        if isinstance(result, tuple):
            ...
            return x + self.dropout(result), None
...
class Attention(nn.Module):
    ...
    def forward(self, x, task=None):
        ...
        if task <= 3:
            ...
            task_prefix_tensor = self.conPerfix.down.weight
...
def model_init(model):
    if hasattr(model, 'use_domain_heads') and model.use_domain_heads:
        # 域分类头模式下，只初始化transformer中的MaskedLinearDynamic层
        weight_masks = []
        bias_masks = []
        
        # 查找HAR_Linear模块
        for module in model.modules():
            if isinstance(module, HAR_Linear):
                # 初始化HAR_Linear中的MaskedLinearDynamic层
                for sub_module in module.modules():
                    if isinstance(sub_module, MaskedLinearDynamic):
                        weight_mask = torch.from_numpy(np.ones(sub_module.weight.shape, dtype=int))
                        weight_masks.append(weight_mask)
                        bias_mask = torch.from_numpy(np.ones(sub_module.bias.shape, dtype=int))
                        bias_masks.append(bias_mask)
                
                # 设置掩码
                if len(weight_masks) > 0:
                    module.set_masks(weight_masks, bias_masks)
                break
