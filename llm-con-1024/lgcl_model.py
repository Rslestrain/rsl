...
class LGCLWrapper(nn.Module):
    ...
    def forward(self, inc_data, task_info=None):
        x = inc_data['x']
        y = inc_data.get('y', None) # 使用 .get() 方法，如果'y'不存在，则返回None，不会报错
        ...
        logits, visual_features, task_prefix, domain_logits, distillation_loss, *rest = outputs
        
        # 如果有蒸馏损失，它会是 rest 列表的第一个元素
        distillation_loss = rest[0] if rest else None
        ...
        return logits, loss_class, loss_task, loss_domain_align, domain_logits, distillation_loss