...
def main():
    ...
    # 直接创建 HARTrans 模型。
# HARTrans 内部的 __init__ 方法会根据配置加载新的 LearnablePromptDistillation 模块。
    model = HARTrans(args) 
...
def train(args, agent, train_loader, eval_loader, train_sampler, test_sampler, device, mylogger):
    ...
    if not (hasattr(args, 'use_domain_prediction') and args.use_domain_prediction):
        agent.model.prototypes = agent.compute_prototypes(
    ...
    for task in range(start_task, args.n_tasks):
        ...
        for epoch in range(n_epochs):
            ...
            if (epoch + 1) % 1 == 0 or (epoch + 1 == n_epochs):
                ...
                if acc > bestacc:
                    bestacc = acc
                    epoch_best_model = {k: v.cpu() for k, v in agent.model.state_dict().items()} 
                agent.train()

        best_accs.append(round(bestacc, 2))
        if epoch_best_model:
            agent.model.load_state_dict(epoch_best_model)
            agent.model.to(device) # 确保模型回到GPU 
