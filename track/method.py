...
class Method(nn.Module):
    ...
    def observe(self, inc_data, task_info, freeze_masks):
        ...
        # 1. 模型前向传播
        inc_data['labels'] = inc_data['y']  # 添加标签信息
        outputs = self.model(inc_data, task_info)
        
        # 【保持不变】: 这里的解包现在是正确的了，因为 LGCLWrapper 返回了6个值
        logits, loss_class, loss_task, loss_domain_align, domain_logits, distillation_loss = outputs
        ...
        if self.args.lgcl_enabled:
            ...
            delta = getattr(self.args, 'domain_prediction_weight', 0.1)
            
            total_loss += alpha * loss_class + beta * loss_task
            
            if hasattr(self.args, 'use_domain_alignment') and self.args.use_domain_alignment:
                total_loss += gamma * loss_domain_align
            
            if hasattr(self.args, 'use_domain_prediction') and self.args.use_domain_prediction:
                total_loss += delta * loss_domain_pred
            if distillation_loss is not None:
                distillation_weight = getattr(self.args, 'distillation_weight', 0.2)
