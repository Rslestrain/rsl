...
def compute_average_activation(network, train_loader, dev, task=None):
    total_activations = []
    network.train()
    with torch.no_grad():
        for batch_idx, batch in enumerate(train_loader):
            # 处理可能返回2个或3个值的情况
            if len(batch) == 3:
                data, target, _ = batch  # 忽略person信息
            else:
                data, target = batch
                
            data = data.to(dev)
            _, activations = network.forward_activations(data,task)
...
def compute_average_activation_old(network, train_loader, dev, task=None):
    total_activations = []
    network.train()
    with torch.no_grad():
        for batch_idx, batch in enumerate(train_loader):
...
def compute_freeze_and_drop(stable_indices, model):
    ...
    # 检查是否使用域分类头
    if hasattr(model.consense_model, 'use_domain_heads') and model.consense_model.use_domain_heads:
        # 域分类头模式下，只处理transformer中的MaskedLinearDynamic层
        for module in model.consense_model.transformer.modules():
            if isinstance(module, MaskedLinearDynamic):
                ...
                if weight_mask is not None and bias_mask is not None:
                    ...
                    # 如果掩码未初始化，创建默认的全1掩码
                    weight_mask_np = np.ones(module.weight.shape)
                    bias_mask_np = np.ones(module.bias.shape)
                    weight.append((weight_mask_np, bias_mask_np))
    else:
        # 原有模式
    ...
    # 如果没有找到任何MaskedLinearDynamic层，返回None
    if len(weight) == 0:
        return None
    ...
    for i, (source_stable, target_stable) in enumerate(zip(stable_indices[:-1], stable_indices[1:])):
        ...
        #Conv2Linear or Linear2Linear
            ...
            #Conv2Linear
            if i > 0 and len(weight[i-1][0].shape) == 4:
                for src_unit_stable in source_stable:
                    if hasattr(model, 'conv2lin_kernel_size'):
                        conv2lin_kernel_size = model.conv2lin_kernel_size
                    else:
                        conv2lin_kernel_size = 1  # 默认值
