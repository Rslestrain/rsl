...
def train(args, agent, train_loader, eval_loader, device, mylogger):
    ...
    for task in range(start_task, args.n_tasks):
        ...
        if hasattr(args, 'use_ncd') and args.use_ncd:
            ...
            mylogger.info(f"Task {task}: Classes {current_task_classes}, "
            ...
            current_task_classes = list(range(task * classes_per_task, 
        ...
        for epoch in range(n_epochs):
            for i, batch in enumerate(train_loader):
                if len(batch) == 3:  # 包含域信息
                    x, y, person = batch
                    x = x.to(device, non_blocking=True)
                    y = y.to(device, non_blocking=True)
                    person = person.to(device, non_blocking=True)
                    inc_data = {"x": x, "y": y, "t": task, "person": person}
                else:
                    x, y = batch
                    x = x.to(device, non_blocking=True)
                    y = y.to(device, non_blocking=True)
